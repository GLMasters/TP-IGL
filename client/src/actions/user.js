...
const resetUserPassword = (password) => async (dispatch, getState) => {
  console.log(getState().userReducer.generatedToken,password);
  const token=getState().userReducer.generatedToken;
  try {
    dispatch({
      type: USER_LOADING,
    });
    const res = await url.post(
      ...
      {
        headers: {
          Authorization: `Bearer ${getState().userReducer.generatedToken}`,
          "Content-Type":"application/json"
        },
      }
    );
